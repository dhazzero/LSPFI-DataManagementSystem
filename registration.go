...
func (a *App) registrationSummary(w http.ResponseWriter, r *http.Request, s Session) {
	...
	certSummary := certificateSummaryInfo{Last: certFloor}
	if certFloor < maxRegistrationSequence {
		certSummary.Next = fmt.Sprintf("[sektor] [profesi] [jenjang] %07d %04d", certFloor+1, year)
	}
